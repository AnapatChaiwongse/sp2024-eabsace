...
class Seq2SeqModel:
    ...
    def train(
        ...
        hist_graph = {
			...
			'train_acc': [], 
			'train_precision': [], 
			'train_recall': [],  # Initialize hist_graph with necessary keys
			# 'val_loss': [],
			'train_f1': [],
         }
        predicted_values = []
        ...
        #        print(loss)
            ...
            if val_accuracy > best_accuracy:
               ...
               with open(f"outputs/Parrot/{fold}-fold/output.txt", 'a') as f0:
            ...
            # ax.plot(hist_graph['train_acc'], label='Train Accuracy')
            # ax.plot(hist_graph['train_precision'], label='Train Precision')
            # ax.plot(hist_graph['train_recall'], label='Train Recall')
            # ax.plot(hist_graph['train_f1'], label='Train F1')
            # ax.set_xlim([0, 5])
            # ax.set_ylim([0, 1])
            # ax.set_ylabel('Acc')
            # ax.set_xlabel('Epochs')
            # plt.legend()
            # plt.savefig("Parrot-1.png")
