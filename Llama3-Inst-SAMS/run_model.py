...
if config.mode != 'cli':
    ...
    if config.mode == 'train':
        ...
        if id_tokenized_ds.get("train") is not None:
            if config.task == 'atsc':
                id_tr_pred_labels = t5_exp.get_labels(tokenized_dataset = id_tokenized_ds,
        ...
        if id_tokenized_ds.get("test") is not None:
            if config.task == 'atsc':
                id_te_pred_labels = t5_exp.get_labels(tokenized_dataset = id_tokenized_ds,
                                                    batch_size = config.per_device_eval_batch_size,
                                                    sample_set = 'test')
            else:
    ...
    if config.task == 'atsc':
        ...
        model_input = bos_instruction_id + config.test_input + f'. The sentence is: {aspect_term}' + eos_instruction
    else:
